...
class QTSegmentExtractor:
    ...
    def extract_one_ecg(self, ecg_id: int):
        if len(str(ecg_id)) == 8:
            pid = int(str(ecg_id)[:-3])
        else:
            pid = int(str(ecg_id)[:-4])

        meta = pd.read_excel(self.metadata_path)
        frequency = int(meta[meta['ECG_ID'] == ecg_id]['Frequency'].values[0])
        segment_dict = self._parse_annotation(ecg_id=ecg_id, pid=pid, frequency=frequency)
        return segment_dict[pid]
    ...
    def extract_segments(self, debug: int = None):
        ...
        if debug is not None:
            n = debug
        for _, i in enumerate(pbar := tqdm(range(n))):
        # for i in range(len(self.ecg_ids)):
            ecg_id = self.ecg_ids[i]
            pid = self.pids[i]
            pbar.set_description(f'Extracting QT segments for PID={pid} ECG={ecg_id}')
    ...
    def _parse_annotation(self, ecg_id: int, pid: int, frequency: int):
        ...
        try:
            ...
            print(f'Error while loading ECG {ecg_id} of PID {pid}: {e}')
            return {}
            ...
            for bound in final_bound_list:
                ...
                if can_extract:
                    ...
                    for lead_id in self.selected_leads:
                        ...
                        qt_onsets_offsets.append((onset, offset))
                        qt_segments.append(lead_qt_segment)
                    qt_segments = np.array(qt_segments)
                    if pid in results:
                        results[pid].append(qt_segments)
                        results_onsets_offsets[pid].append(qt_onsets_offsets)
                    else:
                        results[pid] = [qt_segments]
                        results_onsets_offsets[pid] = [qt_onsets_offsets]

            if pid in results:
                segments = results[pid]
                results[pid] = {'ecg_denoised': ecg,
                                'segments': segments,
                                'qt_onsets_offsets': results_onsets_offsets[pid],
                                'hb_interval': hb_interval,
