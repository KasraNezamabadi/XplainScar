...
class Util:
    ...
    @staticmethod
    def convert_feature_value(feature_name: str, value: float):
        if '_' in feature_name:
            feature_name = feature_name.replace('_', ' ')
        if any([True if f in feature_name else False for f in ['QRS q', 'QRS r', 'QRS s', 'amp', 'prom', 'elevation']]) \
                and 'slope' not in feature_name:
            # The unit should be mV. Convert the micro-volt to milli-volt.
            value /= 1000
        elif 'energy' in feature_name:
            # The unit should be mV^2. Convert the micro-volt to milli-volt.
            value /= 1000000
        elif 'auc' in feature_name:
            # The unit should be mV.second. Convert the micro-volt to milli-volt.
            value /= 1000
